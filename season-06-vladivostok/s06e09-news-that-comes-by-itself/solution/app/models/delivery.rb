...
class Delivery < ApplicationRecord
  ...
  # Табло обновляется само: изменилась поставка — её строка уходит всем, кто
  # смотрит на табло, по кабелю (Action Cable, поток «pier»). Новая
  # квитанция и выплата трогают поставку (`touch`, s06e07) — и тоже
  # обновляют её строку.
  #
  # `_later` и `_commit`: строку рисует работник, а не тот, кто правит, и
  # рисует после того, как правка легла. Откатилась правка — никто ничего
  # не увидит.
  PIER = "pier"
