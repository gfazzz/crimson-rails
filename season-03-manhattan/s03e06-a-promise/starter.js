...
// «передаётся».
//
// TODO: state.js — добавить withState(state, id, next) и pending(state)
// TODO: render.js — отрисовывать состояние "sending" и помечать такую запись
//       занятой (aria-busy), а по ответу пометку снимать
// TODO: спросить линию про все записи смены, кроме уже непрошедших
// TODO: пока ждём — состояние "sending"; ответ применять сразу, как пришёл
// TODO: итог сообщить живой областью смены (#shift-notice), отдельной от
//       области формы подачи
// TODO: у обещания обязан быть .catch или try: отказ, который никто не ловит,
//       — это ошибка, о которой никто не узнает
//
// Внимание: слово wire теперь занято настоящей линией (import из ./wire.js).
// Вспомогательную функцию «повесить однажды» придётся назвать иначе.
//
