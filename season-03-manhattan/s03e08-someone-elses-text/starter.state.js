...
// TODO: у записи появилось поле `source` — ссылка на источник, её присылает
//       пост. Объявить его здесь. Важно, что именно объявить: контора завтра
//       пришлёт поле, о котором мы не договаривались, и оно не должно доехать
//       до страницы само собой.
...
export function read(root) {
  const entries = [...root.querySelectorAll("li[data-id]")].map((node) =>
    entry({
      ...
      // TODO: текст и источник теперь лежат в помеченных местах строки,
      //       а не в «первом попавшемся <p>». Прочитать их оттуда.
