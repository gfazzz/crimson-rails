...
// TODO: отбор берут из адреса, а не из разметки: разметка говорит, что в
//       журнале, адрес — как на него смотрят. И управление приводят к виду,
//       а не наоборот: читатель мог прийти по ссылке.
...
// TODO: единственное место, где меняется вид. Состояние и адрес переставлять
//       вместе: порознь они разойдутся в первый же день.
function look(filter) {
  update(withFilter(state, filter));
}

// TODO: «назад» — это не отмена действия, а другой вид. Применить его, но не
//       писать обратно в историю: запись в ответ на чтение — петля, из
//       которой читатель не выберется кнопкой. И привести поле отбора.
