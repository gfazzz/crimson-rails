...
// У записи есть форма, и только то, что в ней объявлено, попадает в состояние.
// Контора завтра пришлёт поле, о котором мы не договаривались; оно не должно
// доехать до страницы само собой. Поле `source` — ссылка на источник, её
// присылает пост, и это адрес, написанный не нами. Поле `at` — время приёма;
// его ставит контора, и оно нужно живому «сколько прошло».
export function entry({ id, state = "sent", hand = null, text = "", source = null, at = null, chalk = false }) {
  return { id: String(id), state, hand, text, source, at, chalk: Boolean(chalk) };
}
...
export function read(root) {
  const entries = [...root.querySelectorAll("li[data-id]")].map((node) =>
    entry({
      ...
      text: (node.querySelector("[data-part='text']")?.textContent ?? "").trim(),
      source: node.querySelector("[data-part='source']")?.getAttribute("href") ?? null,
      at: node.querySelector("[data-part='at']")?.getAttribute("datetime") ?? null,
