...
//
// Редакция девятая: часть работы отдана своим элементам. Живая область смены
// теперь элемент разметки, а не узел, созданный модулем: объявлена живой она
// с самого начала, а не вместе с первым сообщением.
//
...
// TODO: подключить elements.js — теги определяются до первой отрисовки
...
// TODO: живая область смены лежит в разметке и умеет говорить сама.
//       Найти её здесь.
...
once(compose, "wiredSend", () => {
  compose.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      // TODO: смена продолжается — следующую запись передают, не трогая мышь.
    } catch (error) {
...
function notify(text) {
  // TODO: сказать это живой области. Сообщить — не значит увести: фокус
  //       читателя остаётся там, где был.
}
