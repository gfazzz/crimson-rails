...
function renderEntries(root, state) {
  ...
  // TODO: строка, которую снимают со страницы или прячут отбором, может
  //       держать фокус читателя. Узел уйдёт, фокус уйдёт вместе с ним — на
  //       <body>, то есть в начало документа. Тот, кто читает страницу
  //       голосом, окажется в самом верху и без объяснения.
  //       Заметить это до того, как узел исчез, и поставить фокус на журнал.
...
function fill(node, item) {
  ...
  // TODO: время приёма отдать своему элементу атрибутом datetime. Что он с
  //       ним сделает — его дело: отрисовка про минуты и часы ничего не знает.
  //       Времени нет — атрибута тоже: отрисовка приводит дерево к состоянию.
