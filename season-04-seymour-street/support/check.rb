...
module Crimson
  ...
  class Test < Minitest::Test
    ...
    # после каждого шага.
    # Обратимость набора миграций. Проверяется не состав таблиц целиком —
    # сезон кумулятивен, и дальше их станет больше, — а три свойства, которые
    # остаются верными всегда: свои таблицы поднимаются, откат не оставляет
    # ничего, обратный прогон возвращает то же самое.
    def assert_reversible(*expected)
      steps = migrate_down_and_up
      expected.flatten.map(&:to_s).each do |name|
        assert_includes steps[:up], name, "После прогона таблицы #{name} нет."
      end
      assert_equal [], steps[:down],
                   "После отката до нуля в схеме осталось: #{steps[:down].join(', ')}. " \
                   "Миграция, которую нельзя откатить, превращает любую ошибку в схеме в " \
                   "ручную работу на живой базе."
      assert_equal steps[:up], steps[:again],
                   "Обратный прогон дал другую схему: откат обязан возвращать ровно то " \
                   "состояние, из которого откатывали."
    end
